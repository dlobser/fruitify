...
TREE.prototype.solidify = function(geo,offset,w,h){
	...
	for (var i = facesize; i < geo.faces.length; i++) {
		...
		var temp = geo.faces[i].a;
		geo.faces[i].a = geo.faces[i].c;
		geo.faces[i].b = geo.faces[i].b;
		geo.faces[i].c = temp;
	...
	function computeFaceNormals(fc,v) {

		var cb = new THREE.Vector3(), ab = new THREE.Vector3();

		for ( var f = 0, fl = fc.length; f < fl; f ++ ) {
	...
	// computeFaceNormals(newFaces,geo.vertices);
