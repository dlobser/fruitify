...
function upResAndSave(name,toSave){
	...
	// if(!saveMe)
	returner = saverX3D(name,saveMe);

	return returner;
};

function saverX3D(name,toSave) {

	var scaleOut = 4;//outputScale || 1;
	var saveMe = toSave?true:false;
	var name = name || "Fruitify.x3d";
...
THREE.saveGeometryToX3D = function (geo,nums,scalar) {
	...
	for (i = 0; i < geo.geometry.faces.length; i++) {
		...
		s[2]+=geo.material.color.r.toFixed(2) + ' ' + 
		geo.material.color.g.toFixed(2) + ' ' + 
		geo.material.color.b.toFixed(2) + ' 1 ';
